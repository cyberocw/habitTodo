...
/**
 * Created by cyber on 2017-08-26.
 */
...
public class ZipUtils {
    ...
    public static void zip(String sourcePath, String output) throws Exception {
        zip(new File(sourcePath), new File(output));
    }
    public static void zip(File sourceFile, File output) throws Exception {
        // 압축 대상(sourcePath)이 디렉토리나 파일이 아니면 리턴한다.

        //File sourceFile = sourcePath;//new File(sourcePath);
     ...
     * @throws Exception
     */
    private static void zipEntry(File sourceFile, ZipOutputStream zos) throws Exception {
        ...
        if (sourceFile.isDirectory()) {
            ...
            for (int i = 0; i < fileArray.length; i++) {
                zipEntry(fileArray[i], zos); // 재귀 호출
            }
